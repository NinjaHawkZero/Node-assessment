...
describe("GET /users", function() {
  ...
  test("Token object should be labled correctly", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect({ _token: tokens.u1 }).toBe({token: tokens.u1});
    expect(response.body.users.length).toBe(3);
  });
});
...
describe("PATCH /users/[username]", function() {
  ...
  test("should deny access if not admin/right user", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ token: tokens.u2 }); // wrong user!
    expect(response.statusCode).toBe(401);
  });
  ...
  test("should patch data if admin", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ token: tokens.u3, first_name: "new-fn1" }); // u3 is admin
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
...
//Test for Bug #3
  test("should patch data if user and not admin", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ token: tokens.u1, username: "u1", first_name: "new-fn1" }); // u1 is not admin
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
  ...
  //Test for Bug #2
  test("should send not authorized", async function() {
    const response = await request(app)
      .patch("/users/u1")
  ...
  test("should disallowing patching not-allowed-fields", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ token: tokens.u1, admin: true });
    expect(response.statusCode).toBe(401);
  });
  ...
  test("should return 404 if cannot find", async function() {
    const response = await request(app)
      ...
      .send({ token: tokens.u3, first_name: "new-fn" }); // u3 is admin
...
describe("DELETE /users/[username]", function() {
  ...
  test("should allow if admin", async function() {
    const response = await request(app)
      .delete("/users/u1")
      .send({ token: tokens.u3 }); // u3 is admin
    expect(response.statusCode).toBe(200);
